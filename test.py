...
decomposition = seasonal_decompose(shiftted['load'],freq=30)
...
#Arima_model = auto_arima(data, start_p=1, start_q=1, max_p=4, max_q=4, start_P=0, start_Q=0, max_P=3, max_Q=3, m=24,
#                         seasonal=True, trace=True, error_action='warn', suppress_warnings=True, random_state=20,
 #                        n_fits=20)
...
# import statsmodels.api as sm
#
# # model = sm.tsa.statespace.SARIMAX(train['load'], order=(1, 1, 10), seasonal_order=(2, 0, 2, 24), enforce_stationarity=True,enforce_invertibility=False)
# # fitted = model.fit()
# # print(fitted.summary())
# #
# # forecast = round(fitted.forecast(steps=3018), 2)
# # forecast = pd.DataFrame(forecast.values, index=test.index)
# # print('r2_score is %4f' % r2_score(test['load'], forecast))
# # plt.figure(figsize=(11, 9))
...
# df_consum['temperature'] = data.temperature[24:]
#
# # In[24]:
#
#
...
#
#
# input_ = Input(shape=(5,), dtype='float32')
