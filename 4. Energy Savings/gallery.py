...
if __name__ == '__main__':
    ...
    # plt.show()
        ...
        #<==== This adds a graph above the first row (index=1)
...
class MyPopup(Popup):
    ...
    def droite(self):
        ...
        if (j) != len(L):
            self.source= L[j]
            MyPopup(source = self.source, title = self.source).open()
...
class PhotoButton(Button):
    ...
    def open(self):
        content = MyPopup(source = self.source, title = self.source)
        content.open()
...
class Gallery(BoxLayout):
    def __init__(self):
        ...
        dir =['red.jpg','thermo2.png']
        for image in dir:
            L.append(image)
            picture = PhotoButton(source = image )
            self.ids.espace.add_widget(picture)

        n=36

        plt.figure()
        plt.plot(CCO2[24*n:(n+1)*24],color='blue',label='CO2')
        ax = plt.gca()
        ax.set_facecolor('xkcd:grey' )
        ax.legend()
        plt.axvline(x=12, color='green', linewidth=20, alpha=0.7)
        plt.grid('on')

        canvas = FigureCanvasKivyAgg(plt.gcf())
        self.ids.espace10.add_widget(canvas)

        plt.figure()
        plt.plot(Toffice_reference[24*n:24*(n+1)],color='red',label='Temperature')
        ax = plt.gca()
        ax.set_facecolor('xkcd:grey')
        ax.legend()
        plt.axvline(x=12, color='green', linewidth=20, alpha=0.7)
        plt.grid('on')
        canvas2 = FigureCanvasKivyAgg(plt.gcf())
        self.ids.espace11.add_widget(canvas2)

        plt.figure()
        plt.plot(occupancy[24*n:24*(n+1)],color='blue',label="Occ")
        ax = plt.gca()
        ax.set_facecolor('xkcd:grey')
        ax.legend()
        plt.axvline(x=12, color='green', linewidth=20, alpha=0.7)
        plt.grid('on')
        canvas3 = FigureCanvasKivyAgg(plt.gcf())
        self.ids.espace12.add_widget(canvas3)

        plt.figure()
        plt.plot(power_heater[24*n:24*(n+1)],color='purple',label='Power')
        ax = plt.gca()
        ax.set_facecolor('xkcd:grey')
        ax.legend()
        plt.axvline(x=12, color='green', linewidth=20, alpha=0.7)
        plt.grid('on')
        canvas4 = FigureCanvasKivyAgg(plt.gcf())
        self.ids.espace13.add_widget(canvas4)
