...
if __name__ == '__main__':
    ...
    model_T.fit(df.drop(['label'],axis=1), np.array(Toffice_reference))
    # # define new data
    #
    # # make a prediction
    ...
    plt.figure()
    ax = plt.gca()
    plt.plot(yhat,color="pink",label='Treal')
    plt.plot(df['label'],label='T_regression')
    ax.legend()
    plt.title('Temperature regression model, R2_score:%f' % r2_score(yhat,df['label']))

    plt.show()
    print(yhat)

    x_q = df.drop(['power_heater'], axis=1)
    model_Q = linear_model.Lasso(alpha=1.0)
    # # fit model
    model_Q.fit(df.drop(['power_heater'],axis=1), np.array(power_heater))
    # # define new data
    #
    # # make a prediction
        ...
        while k<2000:
            if df['occ'][k+1] >= 0.5 :
                ...
                Q = model_Q.predict(np.array(
                    [[Tset], [df['occ'][k + 1]], [df['Tout'][k + 1]], [df['nebulosity'][k + 1]],
                     [T[k]]]).transpose())
                if Q > 1000:
                    q.append(model_Q.predict(np.array(
                        [[Tcomfort_min], [df['occ'][k + 1]], [df['Tout'][k + 1]], [df['nebulosity'][k + 1]],
                         [T[k]]]).transpose()))
                    T.append(Tcomfort_min)
                else:
                    q.append(model_Q.predict(np.array(
                        [[Tset], [df['occ'][k + 1]], [df['Tout'][k + 1]], [df['nebulosity'][k + 1]],
                         [T[k]]]).transpose()))
                ...
                while Tnow < Tcomfort_min:
                    n_preheat +=1
                    print(n_preheat)
                    Tnow = model_T.predict(np.array([[df['occ'][k+n_preheat]],[df['Tout'][k+n_preheat]], [1000],[df['nebulosity'][k+n_preheat]],[Tnow]]).transpose())
                    print(Tnow)
                if n_horizon >= n_preheat:
                    if model_T.predict(np.array([[df['occ'][k+1]],[df['Tout'][k+1]], [0],[df['nebulosity'][k+1]],[T[k]]]).transpose())> Tsetback:
                        T.append(model_T.predict(np.array([[df['occ'][k+1]],[df['Tout'][k+1]], [0],[df['nebulosity'][k+1]],[T[k]]]).transpose()))
                        q.append(0)
                    else:
                        Tset = Tsetback
                        T.append(Tset)
                        q.append(model_Q.predict(np.array([[Tset],[df['occ'][k+1]],[df['Tout'][k+1]],[df['nebulosity'][k+1]],[T[k]]]).transpose()))
                else:
                    Tset = Tcomfort
                    q.append(max(yhat_Q))
                    T.append(model_T.predict(np.array([[df['occ'][k+1]],[df['Tout'][k+1]],[1000],[df['nebulosity'][k+1]],[T[k]]]).transpose()))
            k = k+1
    ...
    plt.figure()
    ax = plt.gca()
    plt.plot(result[1],label="Controller")
    #plt.plot(Toffice_reference[0:2000])
    #plt.plot(occupancy[0:2000])
    plt.plot(power_heater[0:2000],label="True")
    ax.legend()
    ...
    plt.bar('controller',sum(result[1]))
    plt.bar('real',sum(power_heater[0:2000]))
    plt.show()
    #
    #
    #
